...
$countQuery = "SELECT COUNT(*) AS total FROM berita WHERE judul LIKE ? OR isi LIKE ?";
...
// Query untuk menampilkan berita dengan pagination
// Perbaikan: JOIN menggunakan berita_gambar.id_berita
$sql = "SELECT berita.id, berita.judul, berita.isi, berita_gambar.file_path AS gambar, berita.tanggal
        ...
        WHERE berita.judul LIKE ? OR berita.isi LIKE ?
        ORDER BY berita.tanggal DESC LIMIT ? OFFSET ?";
...
<h1 class="text-center text-3xl font-bold text-teal-800">BERITA TERBARU</h1>
<p class="text-center text-lg text-gray-700 mb-6">Desa Ketanen, Kecamatan Panceng</p>
    
    <!-- Form Pencarian -->
    <div class="flex justify-center mb-8">
        <form action="berita.php" method="GET" class="flex">
            <input type="text" name="search" value="<?php echo isset($_GET['search']) ? htmlspecialchars($_GET['search']) : ''; ?>" placeholder="Cari berita..." class="border border-teal-500 px-4 py-2 rounded-l-lg focus:outline-none" />
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        ...
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                ...
                $gambar_url = !empty($row['gambar']) ? "uploads/berita/" . $row['gambar'] : "images/default.jpg";
                echo '<img src="' . $gambar_url . '" alt="' . htmlspecialchars($row['judul']) . '" class="w-full h-48 object-cover rounded mb-4">';
                echo '<h3 class="text-lg font-bold">' . htmlspecialchars($row['judul']) . '</h3>';
                echo '<p class="text-gray-600 text-sm">' . date("d M Y", strtotime($row['tanggal'])) . '</p>';
                echo '<p class="mt-2">' . substr(htmlspecialchars($row['isi']), 0, 100) . '...</p>';
                echo '<a href="detailberita.php?id=' . $row['id'] . '" class="text-teal-600 hover:underline mt-2 block">Baca Selengkapnya &gt;</a>';
                echo '</div>';
            }
        } else {
