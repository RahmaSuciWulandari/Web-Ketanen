...
if (isset($_GET['id_produk'])) {
    ...
    $sql = "SELECT produk.nama_produk, produk.deskripsi, lapak.alamat, lapak.id_lapak, lapak.kontak
            FROM produk 
...
<body class="bg-gray-100 p-6">
    <div class="max-w-5xl mx-auto bg-white p-8 rounded-lg shadow-lg">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div class="grid grid-cols-2 gap-4">
                ...
                while ($gambar = $result_gambar->fetch_assoc()) {
                    $file_path = htmlspecialchars($gambar['file_path']) ?: 'default.jpg';
                    echo '<img src="' . $file_path . '" alt="' . htmlspecialchars($produk['nama_produk']) . '" class="rounded-lg w-full h-64 object-cover shadow-md" />';
                }
                ?>
            </div>
        </div>
        ...
        <!-- WhatsApp Button -->
        <div>
            <a href="https://wa.me/<?php echo htmlspecialchars($produk['kontak']); ?>?text=Halo,%20saya%20ingin%20memesan%20produk%20<?php echo urlencode($produk['nama_produk']); ?>" 
               class="bg-green-500 hover:bg-green-600 text-white px-6 py-3 rounded-lg font-medium shadow-md inline-block mt-4">
                Beli Produk
            </a>
        </div>
    </div>
